...
#include <stdio.h>
#include <string.h>
#include "../include/dynlink_cuda.h"
#if INIT_CUDA_GL
#include "../include/dynlink_cudaGL.h"
#endif
#if INIT_CUDA_D3D9
#include "../include/dynlink_cudaD3D9.h"
#endif
#if INIT_CUDA_D3D11
#include "../include/dynlink_cudaD3D11.h"
#endif
...
#define CONSOLE(fmt,...) wprintf(L##fmt,__VA_ARGS__);
#else
static LPCSTR __CudaLibName = "nvcuda.dll";
#define CONSOLE(fmt,...) printf(fmt,__VA_ARGS__)
#endif
...
#define GET_PROC_EX(name, alias, required)                     \
    ...
    if (alias == NULL && required) {                                    \
        CONSOLE("Failed to find required function \"%s\" in %s\n",       \
               TEXT(#name), __CudaLibName);                                  \
        }
...
#define GET_PROC_EX_V2(name, alias, required)                           \
    ...
    if (alias == NULL && required) {                                    \
        CONSOLE("Failed to find required function \"%s\" in %s\n",       \
               TEXT(STRINGIFY(name##_v2)), __CudaLibName);                       \
        }
